// Fixing the ResultsToExcel plugin error
...
// This function performs all the actual image processing on each individual
// image and saves the results/intermediate images.
...
function processFile(input, output, file) {
	...
	run("Measure");
	run("Read and Write Excel", "no_count_column stack_results file=[" + combined_output + "testing.xlsx] sheet=" + file_type + "DAPI_MGV");
	run("Clear Results");
	selectImage(id_DAPIduplicate);
	run("Histogram");
	...
	selectImage(id_GFP);
	run("Measure");
	run("Read and Write Excel", "no_count_column stack_results file=[" + combined_output + "testing.xlsx] sheet=" + file_type + "GFP_MGV");
	run("Clear Results");
	selectImage(id_GFP);
	run("Histogram");
	...
	roiManager("measure");
	run("Read and Write Excel", "no_count_column stack_results file=[" + combined_output + "testing.xlsx] sheet=" + file_type + "ROI_area_MGV");
	run("Clear Results");
	...
	run("Read and Write Excel", "no_count_column stack_results file=[" + combined_output + "testing.xlsx] sheet=" + file_type + "Maxima");
	run("Clear Results");
	selectImage(id_Particles);
	roiManager("select", FinalRoiIndex);
	run("Make Inverse");
	...
	run("Read and Write Excel", "no_count_column stack_results file=[" + combined_output + "testing.xlsx] sheet=" + file_type + "Particles");
	run("Clear Results");
	close("*");
	roiManager("reset");
}
